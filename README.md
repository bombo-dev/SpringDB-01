...
### 구현체에 종속되는 인터페이스
- 추가적으로 이것도 모르고 있던 부분이었다. 인터페이스를 구현하는 시점에서 만약에 인터페이스를 구현한 구현체에서 예외가 발생해서 예외를 처리하지 않고 던지는 상황이 발생하면 인터페이스에도 해당 예외처리를 던지겠다는 걸 명시해주어야 한다는 것이다. 인터페이스는 구현체를 쉽게 바꾸기 위해서 사용하는 것인데, 이는 인터페이스의 장점에서 벗어나 구현체에 종속이 되어버린다. 물론 예시로 추가적으로 Throws Exception을 통해 예외를 던지도록 선언을 해놓으면, 어떤 자식 예외를 던지더라도 처리할 수 있지만 이것 또한 체크예외의 장점에서 벗어난 행동이기 때문에 피해야 한다.
