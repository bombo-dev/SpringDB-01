...
### 체크 예외 활용 문제
- 대부분의 경우에는 런타임 예외로 해결되지만, 정말 중요한 체크 예외의 장점인 개발자가 대로 까먹어서는 안되는 예외는 체크 예외로 지정을 해주어야 한다. `SQLException`, `ConnectException` 과  같은 예외들은 애플리케이션 로직에서 처리할 수가 없다. 체크 예외를 해결하지 않으면 Throws가 Controller까지 가게되는데, 이는 좋지 않은 현상이다. 따라서 이전에 배운 ControllerAdvice를 사용하거나 API를 사용하는 경우에는 `ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build()` 를 응답을 해주어야 한다. 모든 예외 케이스에 대해서 생각하고 반환하는 연습이 중요하다.
- 두 번째로, 체크 예외가 두 개 발생한다고 해서 상위 계층인 Exception 으로 변경하여 Throws를 하게 되면 한 개 밖에 없어서 그나마 깔끔해보일 수 있는데, 이러면 체크 예외를 쓰는 이유가 사라진다. 체크 예외는 잘 사용하지 않지만 위에서 얘기했듯이 쓰는 이유는 정말로 발생해서는 안되는 예외들에 대해서 컴파일러가 체크할 수 있도록 해주는 것이다. 그런것들을 Exception으로 뭉쳐서 던져버리게 되면 개발자의 입장에서 내가 어떤 예외를 정확하게 처리해야 되는지 모를 것이다. 따라서 해당처럼 작업하는 것은 안티패턴이니 피하도록 하자.

### 체크 예외를 언체크 예외로
- 기존에 체크 예외의 같은 경우에 애플리케이션 단에서 처리를 못한다고 했는데 이번 파트에서는 체크 언어의 예외 트레이스를 런타임 예외(언체크 예외)로 바꿔줌으로써 기존에 throws를 전파하는 문제를 해결한건데, 이렇게 되면 단순히 throws 만을 피하기 위해서 사용하는 것일까 생각이 든다. 어차피 SQLException이나 ConnectException은 해결이 불가능하니까 그냥 넘기는건가? 라는 생각이 먼저 들었다. 해당 내용은 질문을 좀 해봐야겠다. 이러한 이유 때문인지 런타임 예외를 사용하면 문서화를 잘 해야된다고 하는게 체크 예외를 런타임 예외로 바꾼것들이 존재하기 때문에, 개발자가 이런 문서들을 잘 읽고 체크예외가 없는지 살펴봐야 할 것 같다.
> 해당 부분에 대해서 질문을 해보았다. 실제로 API를 처리할 때에도 저런 애플리케이션 단에서 처리할 수 없는 예외의 경우에 Advice를 통해서 예외처리를 따로 더 해준다고 한다.   
> 단 500에러와 함께, 어떤 에러인지에 대해서도 명확하게 명시해주고 문서에 작성한다고 한다.
- 이건 모르고 있던 부분이였는데 `e.printStackTrace`는 `System.out`으로 출력이 된다고 한다. 그 말은 이전에 발생했던 모든 예외 메시지들은 System.out 이었다는 뜻이 된다. 로거를 배우면서 알게된거지만 System.out 은 컴파일 단계에서 체크를 하기 때문에 애플리케이션 실행 시간을 늦추는 원인이 되므로, 최대한 log를 사용하도록 하자.
- 추가적으로 이런 경우도 있구나 하고 생각을 못했는데, stackTrace를 끌고오기 위해서는 트레이스를 꼭 `Throwable` 타입의 매개변수에 넣어주어야 어디서 예외가 발생했는지 알 수 있기 때문에 꼭 넣어주자. 넣지 않으면 예외가 어디서 발생했는지 알 수 없다.

### 구현체에 종속되는 인터페이스
- 추가적으로 이것도 모르고 있던 부분이었다. 인터페이스를 구현하는 시점에서 만약에 인터페이스를 구현한 구현체에서 예외가 발생해서 예외를 처리하지 않고 던지는 상황이 발생하면 인터페이스에도 해당 예외처리를 던지겠다는 걸 명시해주어야 한다는 것이다. 인터페이스는 구현체를 쉽게 바꾸기 위해서 사용하는 것인데, 이는 인터페이스의 장점에서 벗어나 구현체에 종속이 되어버린다. 물론 예시로 추가적으로 `throws Exception`을 통해 예외를 던지도록 선언을 해놓으면, 어떤 자식 예외를 던지더라도 처리할 수 있지만 이것 또한 체크예외의 장점에서 벗어난 행동이기 때문에 피해야 한다.

### 예외의 상속
- 추상화 된 예외를 선언하고, 추상화 된 예외에 대한 서브 예외들에 대해서 추상화 된 예외를 상속받음으로서 계층 형식의 예외로 처리가 가능하다. 굳이 예외를 상속해줘야 하나? 라고 생각했는데, 예시 코드를 작성했을 때 처럼 추상화된 예외를 상속받은 서브 예외에 대해서 catch 예외 처리가 실패했을 경우, 부모 예외를 throw 함으로써 어떤 예외에서 어떻게 발생했는지 trace로 확인이 가능하기 때문에 무조건 해야 된다고 생각이 들었다.

### 예외 속에 예외
테스트 코드를 작성하면서, 예외를 처리하기 위해 예외 복구 아이디에서 추가로 예외 복구 아이디 뒤에 랜덤숫자를 넣어서 예외를 처리하도록 로직을 작성한 코드를 볼 수 있었다.
```java
...
```
한 가지 의문이 int값의 범위를 1만으로 설정하면서 다시 한 번 예외가 발생할 일이 없었는데, 혹시나 운이 나빠서 예외가 또 추가적으로 발생할 경우에 대해서 생각해봤다.
```java
private String generateNewId(String memberId) {
        return memberId + new Random().nextInt(3); // 10000 -> 3 으로 변경
    }
```
그리고 3개의 중복된 아이디를 생성하는 서비스 로직을 실행해보았다. 
첫번째로 든 생각은 예외가 잡힐 때 까지 계속 반복되서 catch로 돌아갈까?   
두번째는 catch문에서 실패하면 다음 예외 처리로 넘어가서 다음 예외로 처리를 할까?   
마지막으로 세번째는 실패하면 해당 메소드를 탈출하고 throws로 던질까?   
...
### 스프링 데이터 접근 예외 계층도
데이터 접근 계층에 예외를 처리하기 위해 `RuntimeException`을 상속받는 `DataAccessException`이 있다.   
여기서 두 개로 갈라지는데, NonTransientDataAccessException, TransientDataAccessException 으로 갈라진다. 앞에 두 글자에 주목을 해야한다.
Transient는 일시적인 이라는 의미가 담겨있다. 그래서 전자는 일시적으로 발생하는 예외가 아닌것, 후자는 일시적으로 발생하는 예외이다.   
몇 가지 예시를 들면 Nontransient 예외에는 데이터 무결성 원칙이 위배 된 것, sql 구문이 오류인 것 등을 예로 들 수 있고,    
Transient 예외는 데이터에 락이 걸려있다거나, 커넥션 풀이 꽉차서 타임아웃이 발생했다거나 나중에 다시 실행하면 실행될 수도 있는 것들을 의미한다.
